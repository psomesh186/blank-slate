...
class Server:
    ...
    def __init__(self, host_name, manager):
            ...
            try:
                ...
                if not data:
                    ...
                    if reply["Signal"] == "name":
                        self.manager.get_screen("HostGame").add_player(reply["name"], player_id)
                    elif reply["Signal"] == "Submission":
                        self.manager.get_screen("PlayGame").receive_word(reply["Word"], player_id)
            except socket.error:
                break
